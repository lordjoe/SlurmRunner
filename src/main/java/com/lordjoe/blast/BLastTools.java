...
//        }
    ...
    public static void mergeXMLFiles(String outFile, File[] inFiles, String OriginalName) {
        try {
            PrintWriter out = new PrintWriter(new FileWriter(outFile));
            String header = readHeader(inFiles[0]);
            int[] hitnum =  new int[1]  ;
            out.print(header);
            for (int i = 0; i < inFiles.length; i++) {
                File inFile = inFiles[i];
                addHits(out, inFile,hitnum);
            }
            out.println(FOOTER);
            out.close();
        } catch (IOException e) {
    ...
    private static void addHits(PrintWriter out, File inFile,int[] hitnum) {
        try {
    ...
    private static String readHeader(File inFile) {
        try {
            StringBuilder sb = new StringBuilder();
            LineNumberReader rdr = new LineNumberReader(new FileReader(inFile));
            String line = rdr.readLine();
    ...
    public static void mergeXMLFiles(String[] args) throws IOException, InterruptedException {
        ...
        if (files == null) {
         ...
         mergeXMLFiles(outfile.getCanonicalPath(),files,null);
